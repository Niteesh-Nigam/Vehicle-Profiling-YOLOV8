...
def main():
    csv_file = './step3_lp_lg_detect.csv'  # Update the path if the CSV is in a different directory
    ...
    out = cv2.VideoWriter('./step3_lp_lg_detect.avi', fourcc, 20.0, (int(cap.get(3)), int(cap.get(4))))
        ...
        if frame_nmr in tracking_results:
            for tracking_id, data in tracking_results[frame_nmr].items():
                ...
                if lg_bbox and len(lg_bbox) == 4 and None not in lg_bbox:
                    ...
                    cv2.putText(frame, lg_name, (lg_x1, lg_y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 0, 255), 2)
...
# def main():
    ...
    #             if lg_bbox and len(lg_bbox) == 4 and None not in lg_bbox:
    #                 lg_x1, lg_y1, lg_x2, lg_y2 = map(int, lg_bbox)
    #                 cv2.rectangle(frame, (lg_x1, lp_y1), (lg_x2, lg_y2), (0, 0, 255), 2)
    #                 cv2.putText(frame, "LG", (lg_x1, lg_y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 0, 255), 2)
