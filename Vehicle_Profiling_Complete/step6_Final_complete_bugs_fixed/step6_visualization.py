...
def read_tracking_data(csv_file):
    ...
    with open(csv_file, mode='r') as file:
        ...
        for row in reader:
            ...
            color_name = row['detected_color'] if 'detected_color' in row else 'Unknown'
...
def main():
    csv_file = './step6_lp_lg_detect.csv'  # Update the path if the CSV is in a different directory
    ...
    out = cv2.VideoWriter('./step6_lp_lg_detect.avi', fourcc, 20.0, (int(cap.get(3)), int(cap.get(4))))
        ...
        if frame_nmr in tracking_results:
            for tracking_id, data in tracking_results[frame_nmr].items():
                ...
                if None not in car_bbox:
                    ...
                    cv2.putText(frame, vehicle_color, (x1, y1 + 75), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 0, 255), 2)
